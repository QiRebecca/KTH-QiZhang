...
def main() -> None:
    ...
    if cfg["ar"].get("backend") != "hashed_ridge_smoke":
        ...
        model = train_lora_ar([texts[meta[i]["activation_id"]] for i in idx], subset(h, idx), cfg, ar_artifact_path(cfg, "ar_eval"), seed)
        print(f"trained independent LoRA AR_eval on {len(idx)} activations -> {model.path}")
        return
    meta, h = read_vectors(cfg)
    texts = load_bootstrap_map(cfg)
    idx = split_indices(meta, "train")
    model = train_hashed_ridge([texts[meta[i]["activation_id"]] for i in idx], subset(h, idx), cfg)
    model.save(ar_artifact_path(cfg, "ar_eval"))
    print(f"trained independent AR_eval on {len(idx)} train activations")
